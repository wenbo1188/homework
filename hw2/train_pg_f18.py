...
class Agent(object):
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            # sy_sampled_ac = [0, 0]
            # index = np.argmax(sy_logits_na)
            # sy_sampled_ac[index] = 1
        else:
            # raise NotImplementedError
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            ...
            sy_mean, sy_logstd = policy_parameters
            # YOUR_CODE_HERE
            # sy_logprob_n = -0.5 * tf.squared_difference(sy_mean, sy_ac_na)
    ...
    def sample_trajectories(self, itr, env):
        ...
        paths = []
        while True:
            animate_this_episode=(len(paths)==0 and self.animate)
    ...
    def sum_of_rewards(self, re_n):
        ...
        if self.reward_to_go:
            ...
            for reward_path in re_n:
                q = [0 for i in range(len(reward_path))]
                q[-1] = reward_path[-1]
                for i in reversed(range(len(reward_path) - 1)):
                    q[i] = reward_path[i] + self.gamma * q[i + 1]
                R.extend(q)
            q_n = np.array(R)
        else:
            # raise NotImplementedError
            R = []
            for reward_path in re_n:
                reward = sum([pow(self.gamma, t) * reward_path[t] for t in range(len(reward_path))])
                t = 0
                for r in reward_path:
                    R.append(reward)
            q_n = np.array(R)
            # print(q_n.shape)
    ...
    def compute_advantage(self, ob_no, q_n):
        ...
        if self.nn_baseline:
            ...
            b_n = norm(output_baseline, np.mean(q_n), np.std(q_n)) # YOUR CODE HERE
            adv_n = q_n - b_n
        else:
            adv_n = q_n.copy()
        return adv_n
