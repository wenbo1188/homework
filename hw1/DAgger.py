...
def load_expert_data():
    with open(os.path.join("expert_data", "Walker2d-v2" + ".pkl"), "rb") as f:
...
def main():
    ...
    print(obs[0].shape)
    print(act[0].shape)
    train_datas = obs[:60000]
    train_labels = np.reshape(act[:60000], (60000, 6))
    test_datas = obs[60000:]
    test_labels = np.reshape(act[60000:], (10000, 6))
    
    # print(train_labels[:2])
    model = keras.models.load_model(os.path.join("model", "Walker2d-v2-model-40000-d64-d64-e100.h5"))
    model.compile(optimizer=tf.train.AdamOptimizer(),
                  loss='mse',
                  metrics=['accuracy'])
